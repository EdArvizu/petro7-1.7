function CInterface(){
    ...
    this._init = function(){
        ...
        _oTextHead = new createjs.Text(TEXT_HEAD," 0px "+PRIMARY_FONT, "");
        _oTextHead.x = 0;
        _oTextHead.y = 0;
        ...
        for (var i=0; i<3; i++){
            ...
            for (var j=0; j<3; j++){
                _aTextNumber[i][j] = new createjs.Text();
        ...
        _oBetText.shadow = new createjs.Shadow("", 2, 2, 2);
        s_oStage.addChild(_oBetText);
        
        var oSprite = s_oSpriteLibrary.getSprite('plus_display');
        _oBetDisplay = new CTextToggle(pBetPos.x,pBetPos.y,oSprite,BET[0].formatDecimal(2, ".", ",")+TEXT_CURRENCY,PRIMARY_FONT,"",0,true,s_oStage);        
        ...
        _oButMinus = new CTextToggle(pBetPos.x - 86,pBetPos.y,oSprite,TEXT_MINUS,PRIMARY_FONT,"#ffffff",0,false,s_oStage);
        ...
        var oSprite = s_oSpriteLibrary.getSprite('but_plus');
        _oButPlus = new CTextToggle(pBetPos.x + 86,pBetPos.y,oSprite,TEXT_PLUS,PRIMARY_FONT,"#ffffff",0,false,s_oStage);
        ...
        var oSprite = s_oSpriteLibrary.getSprite('plus_display');
        _oCreditNum = new CTextToggle(pCreditPos.x,pCreditPos.y,oSprite,CREDIT.formatDecimal(2, ".", ",")+TEXT_CURRENCY,PRIMARY_FONT,"red",0,true,s_oStage);        
...
//        _oTotWinNum.block(true);
    ...
    this.refreshBet = function(szValue){
        _oBetDisplay.setText(szValue.formatDecimal(0, ".", ",")+TEXT_CURRENCY);
    };
    
    this.refreshCredit = function(iCredit){
        _oCreditNum.setText(iCredit.formatDecimal(0, ".", ",")+TEXT_CURRENCY);
    };
    
    this.refreshTotWin = function(iTotWin){
        _oTotWinNum.setText(iTotWin.formatDecimal(0, ".", ",")+TEXT_CURRENCY);
        if(iTotWin > 0){
    ...
    this.enablePlayAgain = function(bVal){
        if(bVal){
            var oSprite = s_oSpriteLibrary.getSprite('ready_lose');
            _oButPlayAgain = new CTextToggle(CANVAS_WIDTH/2,500,oSprite,TEXT_PLAY_AGAIN,PRIMARY_FONT,"white",30,false,s_oStage);
            _oButPlayAgain.addEventListener();
        } else {
           ;
        }
